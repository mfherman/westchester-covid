...
p <- mun_sd %>% 
  ggplot(aes(poverty_pct, total_cases_per_cap * 1e5)) +
  geom_point(
    aes(
      size = total_pop,
      text = paste0(
  ...
  scale_y_continuous(label = label_number_si(1)) + 
...
p2 <- mun_sd %>%
  ggplot(aes(crowded_pct, total_cases_per_cap * 1e5)) +
  geom_point(
    aes(
      size = total_pop,
      text = paste0(
...
mun_acs_calc %>%
  ggplot(aes(over_65_pct, total_cases_per_cap)) +
