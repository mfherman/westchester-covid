...
*Most recent data: `r month_day_year(max(mun_recent$date), abbr = TRUE)`*
...
all_days_mun <- crossing(date = all_dates, municipality = all_mun) %>% 
  ...
  ungroup() %>% 
  mutate(
    across(c(active_per_cap, total_cases:active_per_cap),
      ~ if_else(between(date, as.Date("2020-12-12"), as.Date("2020-12-21")), NA_real_, .x)
    ))
  
p <- all_days_mun %>%  
  filter(date >= as.Date("2020-05-05")) %>% 
  mutate(municipality = fct_reorder(municipality, active_per_cap, last)) %>% 
...
ggplotly_config(p) %>% 
  layout(
    annotations = list(
      x = 1, y = -0.075,
      text = 'Source: <a href="https://westchester-covid.mattherman.info/about#covid-19-confirmed-cases" target="_self">Westchester County DOH</a>', 
      showarrow = FALSE,
