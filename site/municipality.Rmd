...
all_days_mun <- crossing(date = all_dates, municipality = all_mun) %>% 
  ...
  mutate(
    ...
    active_per_cap_limit = if_else(active_per_cap_est / 14 * 1e5 > 250, 250, active_per_cap_est / 14 * 1e5)
    ) %>% 
  ungroup()
...
p <- all_days_mun %>%  
  ...
  scale_x_date(
    ...
    breaks = "1 month",
    expand = expansion(mult = c(0.01, 0)),
    limits = c(as.Date("2021-09-01"), NA)
    ) +
  labs(x = NULL, y = NULL, title = "Avg daily cases per 100k (beginning Sept 2021)") +
  theme_minimal() +
  theme(
    legend.position = "none",
    panel.grid = element_blank(),
