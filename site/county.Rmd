...
leaflet(to_map) %>%
  ...
  addLayersControl(
    baseGroups = c("Cases per 100k", "NYS micro-clusters"),
...
wch_nh_sum <- nh_deaths %>% 
  st_drop_geometry() %>% 
  group_by(date) %>% 
  summarize(n = n(), deaths = sum(deaths_total))
...
As of `r month_day_year(wch_nh_sum$date, abbr = TRUE)`, there have been `r comma(wch_nh_sum$deaths + 2, 1)` confirmed or presumed deaths to due Covid-19 in nursing homes in Westchester County. This is `r percent((wch_nh_sum$deaths + 2) / wch_deaths$total_deaths, 1)` of all deaths reported in Westchester.

<aside>
These figures undercount the total deaths of nursing home residents because the NYS DOH only reports deaths that occurred in nursing homes. If a nursing home resident died in a hospital, it is not counted as a nursing home death.
</aside>
