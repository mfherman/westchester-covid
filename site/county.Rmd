  ...
  mutate(
    ...
    label = if_else(
      ...
      paste0(
        ...
        "Onsite students :", comma(students, 1), "<br>",
        "Onsite staff: ", comma(staff, 1), "<br>"
        ),
      paste0(
...
The table below shows the total number of student and staff cases reported by each school since September, as well as the the number of recent cases (within the last 14 days). Each school also reports their teaching model (in-person, hybrid, or remote-only) as well as the number of students and staff currently attending or working in school buildings.
...
school_geo_cases %>%
  ...
  datatable(
    ...
    caption = tags$caption(
      style = 'caption-side: bottom; text-align: right;',
      withTags(div(HTML('Source: <a href="https://westchester-covid.mattherman.info/about" target="_self">NYS Covid-19 Report Card</a>')))
    ),    
    options = list(
      dom = "ft",
      deferRender = TRUE,
      ...
      columnDefs = list(
        list(visible = FALSE, targets = 0),
        list(className = "dt-head-left", targets = 1:4),
  ...
  formatRound(c(5:10), digits = 0) %>%
  formatStyle(1:4, fontFamily = "IBM Plex Sans") %>% 
  formatStyle(5:10, fontFamily = "Roboto Mono")
```
</div>
