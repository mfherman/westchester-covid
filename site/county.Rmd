...
death_by_race <- tribble(
  ~"race",   ~"deaths", ~"pop", ~"age_adjust",
  "Hispanic", 275,      243261, 158.6,
  "Black",    252,      138566, 156.8,
  "White",    729,      520628, 74,
  "Asian",    34,       63448,  51.4
  ) %>% 
  mutate(crude = deaths / pop * 1e5)
...
p <- death_by_race %>% 
  ...
  theme(
    panel.grid.major.x = element_blank(),
    axis.text.x = element_text(size = 15, family = "IBM Plex Sans Condensed"),
    plot.title = element_text(face = "bold", size = 20)
    ) 
